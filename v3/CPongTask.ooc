use sdl,glew
import glew,ITask,sdl/Event,CGlobalTimer,CInputTask,CKernel
...
CPongTask:class extends ITask {
	...
	update: func {
		...
		if( ballX < ballSize ) ballVX = abs(ballVX)
		if( ballX > 1 - ballSize )ballVX = -abs(ballVX)
		...
		if(ballX > p2pos) p2pos += 0.3*CGlobalTimer get() dT
		if(ballX<p2pos) p2pos-=0.3*CGlobalTimer get() dT
